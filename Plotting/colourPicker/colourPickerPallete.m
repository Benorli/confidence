...
function colourPickerPallete()

% Parameters --------------------------------------------------------------
% File to take palettes from
RESOURCE_FNAME = 'colourPicker.mat';
...
TYPE_SPACING = 30;
PALETTE_SPACING = 20 + ~INCLUDE_INTERP * 20;
SUB_PALETTE_SPACING = 5;
BORDER_LEFT = 100;
BORDER_RIGHT = 40;
BORDER_TOP = 50;
BORDER_BOTTOM = 40;
% Order in which to plot the colour scheme types
TYPE_ORDER = {'seq', 'seq2', 'div', 'circ', 'qual'};

% Main --------------------------------------------------------------------
temp = load(RESOURCE_FNAME);
cp = temp.cp;
...
for iType = 1:numel(types)
    npal_per_type(iType) = numel(fieldnames(cp.(types{iType})));
end
...
%     max_ncol_per_type(iType) = max(structfun(@numel, cb.(types{iType})));
    max_ncol_per_type(iType) = max(structfun(@(x) size(x{end}, 1), cp.(types{iType})));
end
...
for iType = 1:numel(types)
    palettes = fieldnames(cp.(types{iType}));
    for iPalette = 1:numel(palettes)
        cmap = cp.(types{iType}).(palettes{iPalette}){end};%  ./ 255;
        axpos = [
